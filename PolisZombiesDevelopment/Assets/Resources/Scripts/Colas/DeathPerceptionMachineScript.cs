...
public class DeathPerceptionMachineScript : MonoBehaviour
{
    public int price;
    ...
    {
        if (other.gameObject.name == "Player" && !other.GetComponent<PlayerScript>().HasCola("DeathPerception") &&
            Input.GetKey("space") && other.gameObject.GetComponent<PlayerScript>().money >= price && other.gameObject.GetComponent<PlayerScript>().playerColas.Count < 3 && timeSinceLastBuy >= timeBetweenBuy)
        {
            other.gameObject.GetComponent<PlayerScript>().money = other.gameObject.GetComponent<PlayerScript>().money - price;
            other.GetComponent<PlayerScript>().AddCola("DeathPerception");
...
void OnTriggerEnter2D(Collider2D other)
{
    if (other.gameObject.tag == "Player")
    {
        ...
        purchaseLabel.GetComponent<TextMeshProUGUI>().text = "purchase cola (" + price + "): Increase vision area";
    }
}
