...
public class RedDotScopeScript : MonoBehaviour             //TODO, esse codigo ainda ta em desenvolvimento. Atualizar ele quando comecar a fazer Attachments.
{
    public int price = 2;
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        ...
        {
            if (!other.gameObject.GetComponent<PlayerScript>().equippedWeaponPrefab.GetComponent<WeaponAcessoriesScript>().scopePrefab.name.Contains("Scope"))
                {
                Debug.Log("magazine");
...
void OnTriggerEnter2D(Collider2D other)
{
    if (other.gameObject.tag == "Player")
    {
        ...
        purchaseLabel.GetComponent<TextMeshProUGUI>().text = "purchase(" + price + "): Better weapon precision";
    }
}
