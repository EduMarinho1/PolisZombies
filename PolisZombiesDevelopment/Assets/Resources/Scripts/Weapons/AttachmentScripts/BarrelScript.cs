...
public class BarrelScript : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        ...
        {
            if (!other.gameObject.GetComponent<PlayerScript>().equippedWeaponPrefab.GetComponent<WeaponAcessoriesScript>().barrelPrefab.name.Contains("Barrel"))
                {
                Debug.Log("barrel");
...
void OnTriggerEnter2D(Collider2D other)
{
    if (other.gameObject.tag == "Player")
    {
        ...
        purchaseLabel.GetComponent<TextMeshProUGUI>().text = "purchase(" + price + "): Increased range of shot";
    }
}
