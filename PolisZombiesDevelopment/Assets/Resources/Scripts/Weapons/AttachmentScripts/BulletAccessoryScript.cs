...
public class BulletAccessoryScript : MonoBehaviour
{
    public int price = 2;
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        ...
        {
            if (!other.gameObject.GetComponent<PlayerScript>().equippedWeaponPrefab.GetComponent<WeaponAcessoriesScript>().bulletPrefabAccessory.name.Contains("Bullet"))
                {
                Debug.Log("Bullet");
...
void OnTriggerEnter2D(Collider2D other)
{
    if (other.gameObject.tag == "Player")
    {
        ...
        purchaseLabel.GetComponent<TextMeshProUGUI>().text = "purchase(" + price + "): Increased damage";
    }
}
