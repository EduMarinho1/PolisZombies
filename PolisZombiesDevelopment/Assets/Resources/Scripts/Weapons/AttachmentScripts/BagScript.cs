...
public class BagScript : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        ...
        {
            if (!other.gameObject.GetComponent<PlayerScript>().equippedWeaponPrefab.GetComponent<WeaponAcessoriesScript>().bagPrefab.name.Contains("Bag"))
                {
                Debug.Log("bag");
...
void OnTriggerEnter2D(Collider2D other)
{
    if (other.gameObject.tag == "Player")
    {
        ...
        purchaseLabel.GetComponent<TextMeshProUGUI>().text = "purchase(" + price + "): Increased total ammo";
    }
}
