...
public class BrazucaExplosionScript : MonoBehaviour
{
    private CircleCollider2D circleCollider;
    void Start()
    {
        PlayerScript playerScript = GameObject.Find("Player").GetComponent<PlayerScript>();
        int brazucaDamage = GameObject.Find("Brazuca").GetComponent<WeaponScript>().damage;
        circleCollider = GetComponent<CircleCollider2D>();
        float explosionRadius = circleCollider.radius * 2f;
        int enemyLayer = 1 << 6;
        int playerLayer = 1 << 3;
        int shockLayer = 1 << 16;
        ...
        foreach (Collider2D hitCollider in hitCollidersEnemies)
        {
            ...
            if (enemy != null)
            {
                ...
                enemy.TakeExplosionDamage(brazucaDamage, 8);
            }
        }
